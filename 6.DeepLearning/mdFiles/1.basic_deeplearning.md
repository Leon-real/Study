...
- [모델준비 클래스화](../jupyter/class_model.ipynb)  
- [Hugging Face 활용](../jupyter/hugging_face.ipynb)  
...
```
- 모델의 마지막 레이어인 Y에 활성화 함수의 경우에 문제에 따라 아래와 같이 사용한다.  
    - 분류 : sigmoid, softmax
    - 회귀 : linear
### 파일 목록  
- 기초
...
my_model.load_weights('my_weights.keras')
```
   
...
for i in range(16):
    ...
    H1 = tf.keras.layers.BatchNormalization()(H1)
    H = tf.keras.layers.Add()([H, H1]) # 이 부분에서 합치려는 레이어 H1은 for loop 전의 dense의 입력값과 같아야 한다.
    H = tf.keras.layers.Activation('swish')(H)
